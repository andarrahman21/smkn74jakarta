...
import { getBerita, getBeritaList, getBeritaSlugs } from "@/lib/queries/berita";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const b = await getBerita(slug);
  if (!b) return { title: "Berita" };
  return {
    title: `${b.title} — Berita SMKN 74`,
    description: b.excerpt,
    openGraph: {
      title: b.title,
      description: b.excerpt,
      images: b.image ? [{ url: b.image, alt: b.imageAlt ?? b.title }] : undefined,
    },
  };
}
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <ViewCounter id={item.id} endpoint="/api/berita/view" />
      <PageHeader
        crumbs={[
          { label: "Beranda", href: "/" },
