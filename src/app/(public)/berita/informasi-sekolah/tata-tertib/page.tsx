  ...
  return (
    <>
      <PageHeader
        ...
        cms={cms}
        cmsPrefix="tatatertib"
      />

      <section className="bg-paper py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <div className="mb-14 reveal">
            <p data-cms-key="tatatertib.pilar_eyebrow" data-cms-type="text" data-cms-label="Pilar — Label" className="text-xs uppercase tracking-[0.22em] text-muted mb-3">{cms["tatatertib.pilar_eyebrow"] ?? "Empat Pilar"}</p>
            <h2 data-cms-key="tatatertib.pilar_heading" data-cms-type="text" data-cms-label="Pilar — Judul" className="font-display headline-section max-w-2xl">{cms["tatatertib.pilar_heading"] ?? "Empat kategori aturan utama."}</h2>
          </div>
            ...
            {groups.map((g, i) => (
              ...
              >
                <div className="flex items-baseline gap-4">
                  <span data-cms-key={`tatatertib.grup_${i}_code`} data-cms-type="text" data-cms-label={`Pilar ${i + 1} — Kode`} className="font-display text-3xl opacity-80">{cms[`tatatertib.grup_${i}_code`] ?? g.code}</span>
                  <h3 data-cms-key={`tatatertib.grup_${i}_title`} data-cms-type="text" data-cms-label={`Pilar ${i + 1} — Judul`} className="font-display text-2xl">{cms[`tatatertib.grup_${i}_title`] ?? g.title}</h3>
                </div>
                <ul className="space-y-2 text-sm leading-relaxed opacity-90">
                  {g.items.map((it, j) => (
                    <li key={it} className="flex items-start gap-2">
                      ...
                      <span data-cms-key={`tatatertib.grup_${i}_item_${j}`} data-cms-type="text" data-cms-label={`Pilar ${i + 1} Poin ${j + 1}`}>{cms[`tatatertib.grup_${i}_item_${j}`] ?? it}</span>
                    </li>
                  ))}
                </ul>
              </article>
      ...
      <section className="bg-white border-t border-black/5 py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <div className="mb-12 reveal">
            <p data-cms-key="tatatertib.sanksi_eyebrow" data-cms-type="text" data-cms-label="Sanksi — Label" className="text-xs uppercase tracking-[0.22em] text-muted mb-3">{cms["tatatertib.sanksi_eyebrow"] ?? "Sanksi & Pembinaan"}</p>
            <h2 data-cms-key="tatatertib.sanksi_heading" data-cms-type="text" data-cms-label="Sanksi — Judul" className="font-display headline-section max-w-2xl">{cms["tatatertib.sanksi_heading"] ?? "Eskalasi yang transparan."}</h2>
            <p data-cms-key="tatatertib.sanksi_body" data-cms-type="textarea" data-cms-label="Sanksi — Deskripsi" className="text-ink/70 leading-relaxed mt-4 max-w-3xl">
              {cms["tatatertib.sanksi_body"] ?? "Sanksi diberikan secara bertahap dengan pendekatan pembinaan terlebih dahulu. Tujuan utama adalah perbaikan perilaku, bukan hukuman."}
            </p>
          </div>
            ...
            {sanksi.map((s, i) => (
              ...
              >
                ...
                <div className="flex-1">
                  <h3 data-cms-key={`tatatertib.sanksi_${i}_level`} data-cms-type="text" data-cms-label={`Sanksi ${i + 1} — Tingkat`} className="font-display text-lg">{cms[`tatatertib.sanksi_${i}_level`] ?? s.level}</h3>
                  <p data-cms-key={`tatatertib.sanksi_${i}_points`} data-cms-type="text" data-cms-label={`Sanksi ${i + 1} — Kategori`} className="text-xs uppercase tracking-widest text-muted mt-1">{cms[`tatatertib.sanksi_${i}_points`] ?? s.points}</p>
                  <p data-cms-key={`tatatertib.sanksi_${i}_body`} data-cms-type="textarea" data-cms-label={`Sanksi ${i + 1} — Keterangan`} className="text-sm text-ink/70 leading-relaxed mt-2">{cms[`tatatertib.sanksi_${i}_body`] ?? s.body}</p>
                </div>
              </li>
            ))}
          </ol>
