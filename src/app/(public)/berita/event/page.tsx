          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {upcoming.map((e, i) => (
              <Link
                ...
                className={`${e.bg} ${e.ink} group reveal relative overflow-hidden rounded-2xl p-7 flex flex-col gap-3 aspect-[4/5] hover:-translate-y-2 transition-transform duration-500`}
              ...
              >
                ...
                <div className="relative z-10 flex items-center justify-between text-[10px] uppercase tracking-[0.22em] opacity-80">
                ...
                <h3 className="relative z-10 font-display text-3xl leading-tight mt-auto">{e.title}</h3>
                <p className="relative z-10 text-sm opacity-85 leading-relaxed">{e.body}</p>
                <div className="relative z-10 border-t border-current/15 pt-3 flex items-center justify-between text-sm">
          ...
          <ul className="space-y-3">
            {past.map((e, i) => (
              <li key={e.slug} className="reveal" style={{ animationDelay: `${i * 0.08}s` }}>
                ...
                >
                  <div className={`${e.bg} ${e.ink} shrink-0 h-16 w-24 rounded-xl grid place-items-center text-[10px] uppercase tracking-widest font-semibold overflow-hidden`}>
                    {e.image ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={e.image} alt={e.imageAlt ?? e.title} className="h-full w-full object-cover" />
                    ) : (
                      e.category
                    )}
                  </div>
