      ...
      <section className="bg-paper py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <div className="mb-12 reveal">
            <p className="text-xs uppercase tracking-[0.22em] text-muted mb-3">Agenda</p>
            <h2 className="font-display headline-section max-w-2xl">Apa yang terjadi setiap hari.</h2>
          </div>
            ...
            {days.map((d, i) => (
              ...
              >
                <div className="shrink-0 md:w-48">
                  <span className="font-display text-3xl text-amber leading-none">Hari {d.n}</span>
                  <p className="text-[10px] uppercase tracking-widest text-muted mt-2">{d.date}</p>
                  <p className="font-display text-xl mt-2">{d.theme}</p>
                </div>
      ...
      <section className="bg-white border-t border-black/5 py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-5 md:px-8 grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-7 reveal">
            <p className="text-xs uppercase tracking-[0.22em] text-muted mb-3">Perlengkapan</p>
            <h2 className="font-display headline-quote mb-6">Yang dibawa setiap hari.</h2>
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {perlengkapan.map((p, i) => (
                <li key={p} className="flex items-start gap-3 p-4 rounded-xl bg-paper-soft">
                  ...
                  <span className="text-sm text-ink/80 leading-relaxed">{p}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="lg:col-span-5 reveal" style={{ animationDelay: "0.1s" }}>
            <div className="bg-navy text-paper rounded-2xl p-7">
              <p className="text-[10px] uppercase tracking-widest text-amber font-medium mb-4">Aturan MPLS</p>
              <ul className="space-y-3 text-sm leading-relaxed">
                {aturan.map((r) => (
                  <li key={r} className="flex items-start gap-3">
                    ...
                    <span className="text-paper/85">{r}</span>
                  </li>
                ))}
              </ul>
            </div>
