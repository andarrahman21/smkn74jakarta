...
export function SearchInput({ defaultValue }: { defaultValue: string }) {
  ...
  return (
    <div className="relative">
      ...
      <input value={value} onChange={(e) => setValue(e.target.value)} placeholder="Cari slide…"
      ...
      {value && (
        <button type="button" onClick={() => setValue("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted hover:text-ink transition-colors">
          <X size={13} />
        </button>
      )}
    </div>
