...
import {
  ...
  duplicateHeroSlide,
  bulkDeleteHeroSlides,
  bulkPublishHeroSlides,
} from "./actions";
import { HeroBannerList } from "./_list";
import { SearchInput } from "./_search";
import { Suspense } from "react";

export const metadata = { title: "Hero Banner — Admin SMKN 74" };

const PAGE_SIZE = 10;

type FilterStatus = "all" | "published" | "draft";
type SortCol = "sort_order" | "status" | "eyebrow";
type SortDir = "asc" | "desc";

const SORT_COLS: SortCol[] = ["sort_order", "status", "eyebrow"];
...
async function getRows(
  ...
  let q = supabase
    .from("hero_slides")
    .select("id, sort_order, eyebrow, head, tag, status", { count: "exact" })
    .order(sort, { ascending: dir === "asc" })
    .range(from, to);

  if (search) q = q.ilike("eyebrow", `%${search}%`);
    ...
    <div className="w-full">
      <header className="mb-6 flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="font-display headline-section">Kelola Hero Banner.</h1>
        </div>
        <div className="flex items-center gap-3">
          <Link
            href="/admin/hero-banner/new"
      ...
      <HeroBannerList
        ...
        sortHrefs={{
          ...
          eyebrow:    buildSortHref({ col: "eyebrow",    sort, dir, search, status: activeStatus }),
          status:     buildSortHref({ col: "status",     sort, dir, search, status: activeStatus }),
        }}
        toggleStatus={toggleHeroSlideStatus}
        duplicate={duplicateHeroSlide}
        deleteFn={deleteHeroSlide}
        bulkDelete={bulkDeleteHeroSlides}
        bulkPublish={bulkPublishHeroSlides}
      />
...
function buildHref({
  ...
  const qs = p.toString();
  return qs ? `/admin/hero-banner?${qs}` : "/admin/hero-banner";
}
