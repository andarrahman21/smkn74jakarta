...
import { createPrestasiCategory, deletePrestasiCategory } from "../actions";
import { DeleteButton } from "@/components/admin/DeleteButton";
import { CategoryForm } from "./_form";

export const metadata = { title: "Kategori Prestasi — Admin SMKN 74" };
...
export default async function Page() {
  ...
  return (
    <div className="w-full max-w-lg">
      ...
      {/* Add form */}
      <CategoryForm action={createPrestasiCategory} />
      ...
      <ul className="mt-6 bg-white border border-black/5 rounded-2xl divide-y divide-black/5 overflow-hidden">
        {(categories ?? []).map((cat) => (
          <li key={cat.id} className="flex items-center gap-3 px-5 py-3">
            ...
            <DeleteButton
              action={deletePrestasiCategory}
              id={cat.id}
              title="Hapus kategori ini?"
              description={`Kategori "${cat.name}" akan dihapus. Prestasi yang sudah menggunakan kategori ini tidak terpengaruh.`}
