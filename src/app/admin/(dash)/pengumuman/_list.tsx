...
import { Loader2, Trash2, CheckCircle, Copy, Check } from "lucide-react";
import * as AlertDialog from "@radix-ui/react-alert-dialog";
import { AlertTriangle } from "lucide-react";
import { DeleteButton } from "@/components/admin/DeleteButton";
import { ChevronUp, ChevronDown } from "lucide-react";

type SortCol = "published_at" | "title" | "category" | "status";
...
export function PengumumanList({
  ...
  function copyPermalink(slug: string, id: string) {
    const url = `${window.location.origin}/pengumuman/${slug}`;
  ...
  return (
    <div>
      {/* Bulk toolbar — only visible when something is selected */}
      {selected.size > 0 && (
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      <AlertDialog.Title className="font-display text-lg leading-snug text-ink">
                        Hapus {selected.size} pengumuman?
                      </AlertDialog.Title>
        ...
        <AlertDialog.Portal>
          ...
          <AlertDialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-[92vw] max-w-md rounded-2xl bg-white p-6 shadow-xl border border-black/10 data-[state=open]:animate-in data-[state=open]:fade-in data-[state=open]:zoom-in-95">
            <div className="flex gap-3">
              ...
              <div>
                <AlertDialog.Title className="font-display text-lg leading-snug text-ink">
                  Nonaktifkan pengumuman ini?
                </AlertDialog.Title>
                <AlertDialog.Description className="mt-1.5 text-sm text-ink/65 leading-relaxed">
                  {nonaktifkanRow && (
                    <><strong className="text-ink">"{nonaktifkanRow.title}"</strong> akan berubah menjadi draft dan tidak tampil di halaman publik.</>
                  )}
                </AlertDialog.Description>
              </div>
            </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-paper-soft">
            <tr>
              ...
              <SortTh col="category" label="Kategori" current={sort} dir={dir} href={sortHrefs.category} className="hidden md:table-cell" />
              <th className="text-left p-3 text-[10px] uppercase tracking-widest text-muted font-medium hidden lg:table-cell">
                Tag
              </th>
              <SortTh col="published_at" label="Tanggal" current={sort} dir={dir} href={sortHrefs.published_at} className="hidden md:table-cell" />
          ...
          <tbody>
            {rows.map((r) => {
              const isScheduled =
                r.status === "published" && new Date(r.published_at) > new Date();

              return (
                <tr
                  key={r.id}
                  className={`border-t border-black/5 transition-colors ${
                    selected.has(r.id) ? "bg-amber/5" : "hover:bg-paper-soft/60"
                  }`}
                >
                  {/* Checkbox */}
                  ...
                  {/* Kategori */}
                  <td className="p-3 align-middle text-sm text-ink/70 hidden md:table-cell whitespace-nowrap">
                    {r.category || "—"}
                  </td>
                  ...
                  <td className="p-3 align-middle hidden lg:table-cell">
                    {r.tag ? (
                      <span className="px-2 py-0.5 rounded-full bg-amber/15 text-amber text-[11px]">
                        {r.tag}
                      </span>
                    ) : (
                  ...
                  <td className="p-3 align-middle">
                    <div className="flex items-center gap-1.5 justify-end">
                      ...
                      <Link
                        href={`/admin/pengumuman/${r.id}/edit`}
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={7} className="p-8 text-center text-sm text-muted">
                  {search
                    ? `Tidak ada pengumuman dengan kata kunci "${search}".`
