...
import { createBeritaCategory, deleteBeritaCategory } from "../actions";
import { DeleteButton } from "@/components/admin/DeleteButton";
import { BeritaCategoryForm } from "./_form";

export const metadata = { title: "Kategori Berita — Admin SMKN 74" };
...
export default async function Page() {
  ...
  return (
    <div className="w-full max-w-lg">
      ...
      <BeritaCategoryForm action={createBeritaCategory} />
      ...
      <ul className="mt-6 bg-white border border-black/5 rounded-2xl divide-y divide-black/5 overflow-hidden">
        {(categories ?? []).map((cat) => (
          <li key={cat.id} className="flex items-center gap-3 px-5 py-3">
            ...
            <DeleteButton
              action={deleteBeritaCategory}
              id={cat.id}
              title="Hapus kategori ini?"
              description={`Kategori "${cat.name}" akan dihapus. Berita yang sudah menggunakan kategori ini tidak terpengaruh.`}
