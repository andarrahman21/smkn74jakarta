...
export function AgendaList({ rows, search, sort, dir, sortHrefs, deleteFn, duplicate, bulkDelete }: Props) {
        ...
        <table className="w-full text-sm">
          <thead className="bg-paper-soft">
            <tr>
              ...
              <SortTh col="scheduled_at" label="Jadwal"   current={sort} dir={dir} href={sortHrefs.scheduled_at} className="hidden md:table-cell" />
              <th className="p-3" />
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              ...
              >
                ...
                <td className="p-3 align-middle">
                  ...
                  {r.location && <p className="text-[11px] text-muted mt-0.5">{r.location}</p>}
                </td>

                {/* Kategori */}
                <td className="p-3 align-middle hidden md:table-cell">
                  {r.category ? (
                    <span className="px-2 py-0.5 rounded-full bg-navy/10 text-navy text-[11px] font-medium">
                      {r.category}
                    </span>
                  ) : (
                ...
                <td className="p-3 align-middle text-sm text-ink/60 hidden md:table-cell whitespace-nowrap">
                  <p>{fmtDate(r.scheduled_at)}</p>
                  <p className="text-[11px] text-muted">
                    {r.time_range || fmtTime(r.scheduled_at)}
                  </p>
                </td>
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={5} className="p-8 text-center text-sm text-muted">
                  {search ? `Tidak ada agenda dengan kata kunci "${search}".` : "Belum ada agenda."}
                </td>
              </tr>
            )}
          </tbody>
