    ...
    <>
        ...
        <div className="mx-auto max-w-7xl px-5 md:px-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {facts.map((f, i) => (
            <div key={f.l} className="reveal" style={{ animationDelay: `${i * 0.06}s` }}>
              <div className="font-display text-4xl text-navy">{f.n}</div>
              <p className="text-[10px] uppercase tracking-widest text-muted mt-2">{f.l}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-paper py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-5 md:px-8 grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-5 reveal">
            <p className="text-xs uppercase tracking-[0.22em] text-muted mb-3">Identitas</p>
            <h2 className="font-display headline-quote mb-4">Sekolah seni dengan akar lokal.</h2>
            <p className="text-ink/75 leading-relaxed">
              Kami berdiri di Jagakarsa, satu kawasan dengan akar seni yang kuat. Kombinasi tradisi dan kontemporer adalah ciri khas pendekatan pembelajaran kami.
            </p>
          </div>
      ...
      <section className="bg-white border-t border-black/5 py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <div className="mb-14 reveal">
            ...
            <h2 className="font-display headline-section max-w-2xl">Tonggak-tonggak perjalanan.</h2>
          </div>
          ...
          <ol className="relative border-l-2 border-amber/30 ml-3 space-y-8">
            {timeline.map((t, i) => (
              <li key={t.year} className="pl-8 relative reveal" style={{ animationDelay: `${i * 0.08}s` }}>
                ...
                <p className="font-display text-3xl text-amber leading-none">{t.year}</p>
                <h3 className="font-display text-2xl mt-2 mb-1">{t.t}</h3>
      ...
      <section className="bg-navy text-paper py-16 md:py-24 relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-5 md:px-8 text-center">
          <p className="text-xs uppercase tracking-[0.22em] text-amber mb-5 animate-fade-up">Komitmen kami</p>
          <h2 className="font-display headline-page font-light animate-fade-up" style={{ animationDelay: "0.15s" }}>
            Mendidik anak-anak yang tahu jalan pulang &mdash;<br />
            <em className="not-italic text-amber">dan jalan bertumbuh.</em>
          </h2>
        </div>
      </section>
